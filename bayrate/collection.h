...
public:
    ...
    void dump_new_ratings( string outfname, string &error);

    void reset();
    void initSeeding(map<int, tdListEntry> &tdList);
    void initSeeding( boost::gregorian::date tournament_date);
...
private:
    void init();
    void storePlayer( const map<string,string> &row, char color);
    float rank2seed( string rank);
    double PI;
